...
**The free build of Claude Code. Stripped all Oauth, Using only API key based.**

This is fork based on [free-code](https://github.com/paoloanzn/claude-code.git) that has all telemetry stripped. All injected security-prompt guardrails removed. All experimental features unlocked. One binary, zero callbacks home. plus for this one, i stripped all oauth including vertex, bedrock, google & only use API_KEY based within .env
...
### 4. Stripped OAuth
This build focused on another model that use API_KEY like openrouter, opencode & others rather only for Anthropic models exclusive.
